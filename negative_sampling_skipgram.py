...
class SkipGramNegSampling(nn.Module):
    ...
    def forward(self, center_idx, pos_idx, neg_idx):
        ...
        pos_embed = self.context_embeddings(pos_idx)
        neg_embed = self.context_embeddings(neg_idx)
        pos_score = torch.sigmoid(torch.matmul(center_embed, pos_embed.t()))
        neg_score = torch.sigmoid(torch.matmul(center_embed, neg_embed.t()) * -1)
...
model = SkipGramNegSampling(len(vocab), embed_size)
...
for epoch in range(epochs):
    total_loss = 0
    for center, context in pairs[:50000]:
        center_idx = torch.tensor([word_to_idx[center]], dtype=torch.long)
        pos_idx = torch.tensor([word_to_idx[context]], dtype=torch.long)
        neg_idx = torch.multinomial(neg_sampling_dist, num_negatives, replacement=True)
        optimizer.zero_grad()
        pos_score, neg_score = model(center_idx, pos_idx, neg_idx)
...
plot_loss(losses, 'Skip-Gram Negative Sampling Loss', 'loss_negative_sampling.png')
visualize_embeddings(model.center_embeddings.weight.detach().numpy(), vocab[:200], 'tsne_negative_sampling.png')
